...
def ejecutar_autoencoder(dataset, n_epochs, batch_size, n_layers_val, n_train, n_test, ansatz, mejora, k, dev_size, lr):
    ...
    theta, phi, params_dec = inicializa_params.inic_params_SQ_global(block_size,  n_layers, n_qubits_total, ansatz)
    ...
    opt = inicializa_params.crear_optimizador_SQ_orig(optimizer_name, theta, phi, params_dec, tasa_de_aprendizaje)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (images, paths) in enumerate(train_loader):
            ...
            for img_idx in range(images.shape[0]):
                ...
                for (i, j) in sampled_blocks:
                    ...
                    loss, mse = funciones_estado.optimizar_autoencoder_bloque_SQ_orig(
                        theta, phi, params_dec, state, autoencoder_circuit, ansatz, mejora
                    )
                    total_img_loss += loss
                    valid_blocks += 1
            ...
            grad_norm = params_dec.grad.norm().item()
        ...
        if avg_dev_loss < best_dev_loss:
            ...
            best_params = copy.deepcopy(params_dec)
        ...
        for f in files_test:
            ...
            for i in range(0, resize_dim[0], block_size): # Iterar sobre la imagen en pasos del tamaño del bloque (filas)
                for j in range(0, resize_dim[1], block_size): # Iterar sobre la imagen en pasos del tamaño del bloque (columnas)
                    ...
                    recon = autoencoder_circuit(state, theta, phi, params_dec, ansatz, mejora)
