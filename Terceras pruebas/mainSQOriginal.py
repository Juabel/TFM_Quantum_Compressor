...
def ejecutar_autoencoder(dataset, n_global, n_local, n_layers_val, n_train, n_test, ansatz, mejora):
    ...
    params = inicializa_params.inic_params_SQ_orig(block_size, resize_dim, n_layers, opt_params, n_qubits_total)
                ...
                for i in range(0, resize_dim[0], block_size):
                    for j in range(0, resize_dim[1], block_size):
                        ...
                        for iter in range(num_iteraciones_bloque):
                            ...
                            # ---- ENTRENAMIENTO ----
                            params[(i, j)], loss, mse = funciones_estado.optimizar_autoencoder_bloque_SQ_orig(
                                opt, params[(i, j)], state, autoencoder_circuit, opt_params, ansatz, mejora
                            )
    ...
    for f in files_test:
        ...
        for i in range(0, resize_dim[0], block_size): # Iterar sobre la imagen en pasos del tamaño del bloque (filas)
            for j in range(0, resize_dim[1], block_size): # Iterar sobre la imagen en pasos del tamaño del bloque (columnas)
                ...
                vals_enc = circuit_enc(state , theta, phi, opt_params)
                ...
                recon = autoencoder_circuit(state, theta, phi, params_dec, opt_params, ansatz, mejora)
    ...
    with open (ruta_log, "a") as f:
        f.write(f"\nMSE medio global de todas las imagenes: {media_final_MSE:.6f}\n")
